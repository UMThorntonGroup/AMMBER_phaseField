...
void customPDE<dim,degree>::setInitialCondition(const dealii::Point<dim> &p, const unsigned int index, double & scalar_IC, dealii::Vector<double> & vector_IC){
    ...
    double x = p[0] - 0.5*userInputs.domain_size[0];
    double y = p[1] - 0.5*userInputs.domain_size[1];
    double z;
    if(dim<3){
      z = 0.0;
    } else {
      z = p[2] - 0.5*userInputs.domain_size[2];
    }
    ...
    double tanh_circle = 0.5*(1.0 - std::tanh(0.5*intf*(r-r0)));
